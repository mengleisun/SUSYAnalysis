...
void testDY(){
  ...
  else if(channel ==2)sigtree->Add("/uscms_data/d3/mengleis/test/resTree_mgsignal_DY.root");
  ...
  else if(channel == 2)controltree->Add("/uscms_data/d3/mengleis/test/resTree_mgsignal_DY.root");

	std::ifstream jetfake_file("JetFakeRate-DY-Sep1.txt");
...
//************ Proxy Tree **********************//
  ...
  if(channel == 1)proxytree->Add("/uscms_data/d3/mengleis/resTree_egsignal_DoubleEG_ReMiniAOD_test_2.root");
	else if(channel == 2)proxytree->Add("/uscms_data/d3/mengleis/test/resTree_mgsignal_DY.root");
...
new_controlPhoEt->Fit("expo");
   ...
   ofstream myfile;
   myfile.open("JetFakeRate-transferfactor-DY-Sep1.txt");
 ...
 TF1 *inifit = new_fakesPhoEt->GetFunction("expo");
   ...
   double iniLambda_num2 = (log(new_fakesPhoEt->Eval(60)- 2*iniCoeff_num*exp(iniLambda_num1*60.0)) - log(new_fakesPhoEt->Eval(120)- 2*iniCoeff_num*exp(iniLambda_num1*120.0)) )/(60.0-120.0);
  	std::cout << "new_fakesPhoEt->Eval(111) = " << log(new_fakesPhoEt->Eval(60)) << "  2*iniCoeff_num*exp(iniLambda_num1*60.0) = " << 2*iniCoeff_num*exp(iniLambda_num1*60.0) << std::endl;
