...
void analysis_T6WG(){//main  
	...
	es->Add("root://cmseos.fnal.gov///store/user/msun/Signal/T6WG_TuneCUETP8M1_RunIISummer16MiniAOD.root");

	RunType datatype(MC); 
	std::ostringstream outputname;
	outputname << "/uscms_data/d3/mengleis/FullStatusOct/resTree_T6WG_debug.root";
  ...
  mgtree->Branch("Mchargino",      &Mchargino);
	...
	for(unsigned ievt(0); ievt<nEvts; ++ievt){//loop on entries
      ...
      //Look for chargino
			...
			if(itMC->getPID() > 1000000 && (fabs(itMC->getMomPID()) < 25 || fabs(itMC->getMomPID())==999) && itMC->getStatus() < 29)ISRVEC.push_back(itMC);
