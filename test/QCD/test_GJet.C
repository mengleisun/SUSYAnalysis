...
void test_GJet(){//main  
  ...
  outputname << "/uscms_data/d3/mengleis/test/resTree_T5WG_GJet.root";
...
//************ Signal Tree **********************//
  ...
  egtree->Branch("isFake",     &eg_isFake);
  ...
  int jetNumber(0);
    ...
    for (unsigned ievt(0); ievt<nEvts; ++ievt){//loop on entries
      ...
      for(std::vector<mcData>::iterator itMC = MCData.begin(); itMC!= MCData.end(); itMC++){ 
          ...
          if(!hasGenPho && itMC->getPID()== 22 && (fabs(itMC->getMomPID()) <=5 || fabs(itMC->getMomPID())==999) ){
             hasGenPho=true;
             genPho = itMC;
      ...
      tree->Fill();
			...
			for(std::vector<recoPhoton>::iterator itpho = Photon.begin() ; itpho != Photon.end(); ++itpho){
				...
				if(GSFveto && PixelVeto && FSRVeto){
					if(!passSigma || !passChIso){
						if( (itpho->getSigma()< 0.02 && itpho->isEB() && itpho->getChIso() < 15) || (itpho->getSigma()< 0.04 && itpho->isEE() && itpho->getChIso() < 15 )  ){fakePho = itpho; hasfakePho = true;}
					}
				}
		...
		//	}
			...
			if(!hasegPho && hasEle && hasfakePho){
				double dReg = DeltaR(fakePho->getEta(), fakePho->getPhi(), egsignalEle->getEta(), egsignalEle->getPhi()); 
				if(dReg>0.8){
						if(((fakePho->getP4()+egsignalEle->getP4()).M() - 91.188) > 10.0){

							float deltaPhi = DeltaPhi(egsignalEle->getPhi(), METPhi);
							float MT = sqrt(2*MET*egsignalEle->getPt()*(1-std::cos(deltaPhi)));
							eg_phoEt = fakePho->getCalibEt();
							eg_phoEta= fakePho->getEta();
							eg_phoPhi= fakePho->getPhi();
							eg_lepPt = egsignalEle->getCalibPt();
							eg_lepEta= egsignalEle->getEta();
							eg_lepPhi= egsignalEle->getPhi();
							eg_sigMT = MT;
							...
							eg_nVertex = nVtx; 
							eg_dRPhoLep= dReg;
							eg_invmass = (fakePho->getP4()+egsignalEle->getP4()).M();
							eg_sigMETJESup = MET_T1JESUp;
							eg_sigMETJESdo = MET_T1JESDo;
							eg_sigMETJERup = MET_T1JERUp;
							eg_sigMETJERdo = MET_T1JERDo;
							...
							for(std::vector<recoJet>::iterator itJet = JetCollection.begin() ; itJet != JetCollection.end(); ++itJet){
								if(!itJet->passSignalSelection())continue;
								if(DeltaR(itJet->getEta(), itJet->getPhi(), fakePho->getEta(),fakePho->getPhi()) <= 0.4)continue;	
								if(DeltaR(itJet->getEta(), itJet->getPhi(), egsignalEle->getEta(),egsignalEle->getPhi()) <= 0.4)continue;
								eg_nJet += 1;
								eg_HT += itJet->getPt();
								eg_HTJESup += itJet->getPt()*(1+itJet->getPtUnc());
