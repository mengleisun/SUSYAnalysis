...
void analysis_T6WG(){//main  
  ...
  es->Add("root://cmseos.fnal.gov///store/user/msun/Signal/SMS-T6Wg_TuneCUETP8M1_RunIISummer16MiniAODv2.root");
...
//************ Signal Tree **********************//
  ...
  int jetNumber(0);
    ...
    for (unsigned ievt(0); ievt<nEvts; ++ievt){//loop on entries
      ...
      float charginoMass(-1), neutralinoMass(-1), squarkMass(-1);
      ...
      for(std::vector<mcData>::iterator itMC = MCData.begin(); itMC!= MCData.end(); itMC++){ 
          ...
          //Look for gluino
          if(isSquark(itMC->getMomPID()) )squarkMass = itMC->getmomMass();
          //Look for neutralino
