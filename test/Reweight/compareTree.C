...
void compareTree(){//main 
  ...
  gSystem->Load("/uscms/home/mengleis/work/SUSY2016/SUSYAnalysis/lib/libAnaClasses.so");
	...
	bool   EBOnly = true;
	//double scalefactor1 = 35.8*1000.0*489.0/6103732; 
	//double scalefactor2 = 35.8*1000.0/27426836; 
	double scalefactor1 = 1; 
	double scalefactor2 = 1; 

	Double_t plotEtBins[]={35,50,100,140,200,250,300,400,600,2000};
	...
	TH1D *p_JetPt_data    = new TH1D("p_JetPt_data","",10,0,1000);
	...
	TH1D *p_JetPt_2    = new TH1D("p_JetPt_2","",10,0,1000);
...
//************ Signal Tree **********************//
  ...
  tree2->Add("/uscms_data/d3/mengleis/test/resTree_ISR_ZG130_NLO.root");
  float second_phoEt(0);
  float second_phoEta(0);
  float second_JetPt(0);
  ...
  tree2->SetBranchAddress("JetPt",    &second_JetPt);
  ...
  for(unsigned ievt(0); ievt<tree2->GetEntries(); ++ievt){//loop on entries
	...
	float scalefactor = p_phoEt_data->Integral(4,9)/p_phoEt_2->Integral(4,9); 
	...
	can_JetPt->cd();
	p_JetPt_data->Draw();
	p_JetPt_2->SetLineColor(kRed);
	p_JetPt_2->Scale(scalefactor);
	p_JetPt_2->Draw("same");
