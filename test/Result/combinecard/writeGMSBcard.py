...
susy_in = ROOT.TFile('../signalTree_M1M2.root','read')
n_channels = 36
...
for i in range(0, h_SUSYmass.GetXaxis().GetNbins() + 1):
    for j in range(0, h_SUSYmass.GetYaxis().GetNbins() + 1):
        ...
        for l in lines:
            if (re.search('NSC', l)
                ...
                for k in range(1, n_channels+1):
                    ...
                    e_jer = h_rates['h_chan' + str(k) + '_syserr_jer'].GetBinContent(i, j)
                    e_esf = h_rates['h_chan' + str(k) + '_syserr_esf'].GetBinContent(i, j)
                    unc_PU = h_rates['m1m2_h_syserr_PU'].GetBinContent(i, j) + 1.0
                    unc_stat = 1.0
                    unc_jes = 1.0 
                    unc_jer = 1.0
                    unc_esf = 1.0
