...
void analysis_W(){//main 

  gSystem->Load("/uscms/home/mengleis/work/SUSY2016/SUSYAnalysis/lib/libAnaClasses.so");

  char outputname[100] = "/uscms_data/d3/mengleis/FullStatusOct/WTree_TT.root";
  ofstream logfile;
  ...
  TChain* es = new TChain("ggNtuplizer/EventTree");
	...
	//es->Add("/uscmst1b_scratch/lpc1/3DayLifetime/mengleis/WJetsToLNu_NLO.root");
...
//************ Signal Tree **********************//
  ...
  sigtree->Branch("nJet",      &nJet);
	...
	sigtree->Branch("WPt",       &WPt);
	sigtree->Branch("TPt",       &TPt);
	if(isMC){
...
//*********** histo list **********************//
        ...
        if(!itpho->passSignalSelection())continue;
			...
			if(hasLep){
				if(raw.passMETFilter(METFilter)){
					...
					phoEta= 0;
					phoPhi= 0;
          double dReg = DeltaR(signalPho->getEta(), signalPho->getPhi(), signalLep->getEta(), signalLep->getPhi()); 
          if(dReg>0.8){
            if(((signalPho->getP4()+signalLep->getP4()).M() - 91.188) > 10.0){
  ...
  mcGMomPID.clear();
					...
					//	if(WPt < 1){
					//	  for(std::vector<mcData>::iterator itMC = MCData.begin(); itMC!= MCData.end(); itMC++){
					//			std::cout << itMC->getPID() << " " << itMC->getMomPID() << " " << itMC->getMass() << " " << itMC->getmomMass() << std::endl;
					//		}
					//	}
