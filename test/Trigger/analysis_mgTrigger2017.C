...
void analysis_mgTrigger2017(){//main  

	gSystem->Load("/uscms/home/mengleis/work/SUSY2016/SUSYAnalysis/lib/libAnaClasses.so");

	char outputname[100] = "/uscms_data/d3/mengleis/FullStatusOct/plot_MuonTrigger2017.root";
	ofstream logfile;
	logfile.open("/uscms_data/d3/mengleis/FullStatusOct/plot_MuonTrigger2017.log"); 
	...
	//es->Add("root://cmseos.fnal.gov//store/group/lpcsusystealth/ggNtuple_leppho/SingleMuon_2017D.root");
	es->Add("/uscmst1b_scratch/lpc1/3DayLifetime/mengleis/trigger.root");
	...
	for (unsigned ievt(0); ievt<nEvts; ++ievt){//loop on entries
		...
		if(ievt%10000==0) std::cout << " -- Processing event " << ievt << std::endl;
			...
			for(std::vector<recoMuon>::iterator itLeadMu = Muon.begin(); itLeadMu!= Muon.end(); ++itLeadMu){
				if(!itLeadMu->fireSingleTrg(31))continue;
            ...
            if(probePho->fireDoubleTrg(29))Z_phofireHLT=1;
						...
						if(probeMu->fireSingleTrg(2))Z_mufireHLT=1;
						else Z_mufireHLT=0;
			...
			if(hasMu && probePhoVec.size() > 0){
				//for(int ipho(0); ipho < probePhoVec.size(); ipho++){
					...
					if(dR > 0.8){ 
						...
						if(itIsoMu->fireSingleTrg(2))mg_mufireHLT=1;
						else mg_mufireHLT=0;
