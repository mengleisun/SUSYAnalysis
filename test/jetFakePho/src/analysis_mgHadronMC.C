...
//************ Signal Tree **********************//
  ...
  TProfile *p_mgTrgEff = new TProfile("p_mgTrgEff","p_mgTrgEff",70,30,100);
  ...
  if(MET>70)continue;
	...
	for(std::vector<recoMuon>::iterator itMu = Muon.begin(); itMu!= Muon.end(); ++itMu){
	   if(itMu->passSignalSelection() && itMu->passHLTSelection()){
		 itLeadMu = itMu; 
		 hasMu=true; 
		 break;
	  }
  ...
  if(!hasMu)continue;
	...
	for(std::vector<recoPhoton>::iterator itpho = Photon.begin() ; itpho != Photon.end(); ++itpho){
     ...
     // bool passDetIso = (itpho->passEcalIso() && itpho->passHcalIso() && itpho->passTrackerIso());
      if(itpho->getR9() < 0.5)isegCandidate = false;
     // if(!passDetIso && itpho->getR9() < 0.85)isegCandidate = false;
      if(!itpho->fireL1Trg(12))ismgCandidate=false;

	  float deltaPhi = DeltaPhi(itLeadMu->getPhi(), METPhi);
	  float MT = sqrt(2*MET*itLeadMu->getPt()*(1-std::cos(deltaPhi)));
      ...
      if(ismgCandidate && !hasmgCandidate){
        ...
        mgtree->Fill();
        if(itpho->getSigma() < 0.014 && raw.run < 278270){
          if((itLeadMu->fireSingleTrg(2) || itLeadMu->fireSingleTrg(21)) && itpho->fireDoubleTrg(28))p_mgTrgEff->Fill(itpho->getCalibEt(),1);
          else p_mgTrgEff->Fill(itpho->getCalibEt(),0);
        }
        hasmgCandidate = true;
      }
