...
void analysis_SUSY(){//main  
  ...
  outputname << "/uscms_data/d3/mengleis/test/resTree_T5WG_test.root";
    ...
    for (unsigned ievt(0); ievt<nEvts; ++ievt){//loop on entries
      ...
      for(std::vector<mcData>::iterator itMC = MCData.begin(); itMC!= MCData.end(); itMC++){ 
          ...
          if(!hasGenPho && itMC->isPhoton() && itMC->decayFromNeu()){
						 std::cout << itMC->getPID() << " " << itMC->getMomPID() << std::endl;
             hasGenPho=true;
             genPho = itMC;
             ...
             if(mindR < 0.15)matchRecoPho=true;
						 ...
						 if(matchRecoPho){
							std::cout << "pixel " << recopho->PixelSeed() << "  iso " << recopho->getChIso() << " rho " << raw.rho << std::endl; 
							bool FSRVeto(true);
							for(std::vector<recoEle>::iterator ie = Ele.begin(); ie != Ele.end(); ie++)
							 if(DeltaR(recopho->getEta(), recopho->getPhi(), ie->getEta(), ie->getPhi()) < 0.3)FSRVeto=false;
							for(std::vector<recoMuon>::iterator im = Muon.begin(); im != Muon.end(); im++)
							 if(DeltaR(recopho->getEta(), recopho->getPhi(), im->getEta(), im->getPhi()) < 0.3 && im->getEt()>2.0)FSRVeto=false;
							if(recopho->getPhoIso() < 3.6+0.0047*genPho->getEt()){
							 std::cout << " FSR " << FSRVeto << " iso " << ((recopho->getChIso() < 1.29)?1:0) << std::endl;
							}
							}
           }//endif: found signal photon 
           //))))))))))))))))))))))
           if(matchRecoPho){
							if(DeltaR(genPho->getEta(), genPho->getPhi(), itMC->getEta(), itMC->getPhi()) < 0.3)std::cout << "suround " << itMC->getPID() << " " << itMC->getMomPID() << " " << itMC->getEta() << " " << itMC->getPhi() << std::endl;
							else if(itMC->getMomPID()== 1000021)std::cout << itMC->getMomPID() << " " << itMC->getmomEta() << " " << itMC->getmomPhi() << std::endl; 
					 }
           ...
           //)))))))))))))))))))))))
			...
			if(matchRecoPho)std::cout << std::endl;
      ...
      tree->Fill();
			...
			if(hasegPho && hasEle){
              ...
              std::cout << "Not T5WG" << std::endl;
      for(std::vector<mcData>::iterator itMC = MCData.begin(); itMC!= MCData.end(); itMC++){ 
				std::cout << itMC->getPID() << " " << itMC->getMass() << " " << itMC->getMomPID() << " " << itMC->getmomMass() << std::endl;
			}}
							egtree->Fill();
