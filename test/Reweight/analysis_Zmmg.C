...
void analysis_Zmmg(){//main  

  gSystem->Load("/uscms/home/mengleis/work/SUSY2016/SUSYAnalysis/lib/libAnaClasses.so");

  char outputname[100] = "/uscms_data/d3/mengleis/test/resTree_ZGamma_data.root";
  ofstream logfile;
  logfile.open("/uscms_data/d3/mengleis/test/resTree_ZGamma_data.log"); 
				...
				if(dRlepphoton > 0.3){
					if(passMETFilter(METFilter)){ 
						...
						for(std::vector<recoMuon>::iterator im = Muon.begin(); im != Muon.end(); im++){
							...
							double llgmass = (im->getP4()+signalLep->getP4()+signalPho->getCalibP4()).M();
			...
			for(unsigned ie(0); ie < proxyLepCollection.size(); ie++){
				...
				if(passMETFilter(METFilter)){
					...
					for(std::vector<recoMuon>::iterator im = Muon.begin(); im != Muon.end(); im++){
						...
						double llgmass = (im->getP4()+jetMuon->getP4()+jetPho->getCalibP4()).M();
						if( (80 < llmass && llmass < 100) || ( 80 < llgmass && llgmass < 100)){
							foundZG = true;
							trailLep = im;
						}
					...
					if(foundZG){
						...
						float ZJet_ThreeBodyMass = (trailLep->getP4()+jetMuon->getP4()+jetPho->getCalibP4()).M(); 
