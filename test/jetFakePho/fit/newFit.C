...
void newFit(int detType){
	...
	for(unsigned ibin(66); ibin <= 85; ibin++){
		...
		for(unsigned j(1); j <=10; j++){
			...
			new_control_error += p_controlPhoEt->GetBinContent(65+(ibin-66)*10+j);
			new_fakes_error += p_fakesPhoEt->GetBinError(65+(ibin-66)*10+j);
			staterror += p_fakesPhoEt_stat->GetBinContent(65+(ibin-66)*10+j);
		}
...
if(channel == 1)fitfunc_num->SetParameters(iniCoeff_num, 650, iniLambda_num1, -0.01);
